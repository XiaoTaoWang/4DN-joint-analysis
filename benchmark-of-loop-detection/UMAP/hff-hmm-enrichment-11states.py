import bisect, random, joblib, os
import numpy as np
...
hmm_fil = os.path.join(chromhmm_folder, 'HFFc6.ChromHMM_8marks_12states.bed')
...
joblib.dump(score_pool, 'HFFc6.hmm-intermediate.by_umap_class.pkl')
'''
score_pool = joblib.load('HFFc6.hmm-intermediate.by_umap_class.pkl')
...
for i in range(6):
    ...
    plt.savefig('HFF.{0}.hmm-fc.svg'.format(i+1), dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.savefig('HFF.FC-features.{0}.svg'.format(i+1), dpi=300, bbox_inches='tight')
    plt.close()
...
plt.savefig('HFF.clusters-features.svg', dpi=300, bbox_inches='tight')
plt.close()
...
for class_label in range(1, 7):
    ...
    for i in idx:
        current_loop_size.append(np.abs(hff_loops[i][4] - hff_loops[i][1]))
...
ax.xaxis.set_tick_params(width=0.4, length=1.5, pad=1, labelsize=5)
ax.yaxis.set_tick_params(width=0.4, length=1.5, pad=1, labelsize=5)
plt.savefig('HFF.loop-median-size.svg', dpi=300, bbox_inches='tight')
plt.close()


fig = plt.figure(figsize=(1.5, 0.75))
ax = fig.add_subplot(111)
data = joblib.load('HFF.contact-strength.ICE.pkl')
normed_data = []
...
ax.xaxis.set_tick_params(width=0.4, length=1.5, pad=1, labelsize=5)
ax.yaxis.set_tick_params(width=0.4, length=1.5, pad=1, labelsize=5)
plt.savefig('HFF.contact-strengths-by-cluster.svg', dpi=300, bbox_inches='tight')
plt.close()