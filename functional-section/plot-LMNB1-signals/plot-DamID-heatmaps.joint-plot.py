...
print(vmin, vmax)
norm = MidpointNormalize(vmin=-1, vmax=1.5, midpoint=0)
...
cbar = fig.colorbar(sc, cax=c_ax, ticks=[-1, 0, 1.5])
...
ax3.fill_betweenx(range(len(exp)), x1=0, x2=np.log2(exp[::-1]+1), color='#E41A1C', edgecolor='none', alpha=0.8)
ax3.tick_params(axis='both', bottom=True, top=False, left=False, right=False,
                labelbottom=True, labeltop=False, labelleft=False, labelright=False)
ax3.spines['top'].set_visible(False)
ax3.spines['right'].set_visible(False)
ax3.set_xlabel('RNA log2(TPM+1)', fontsize=7)
...
ax4.set_xlabel('Gene Length (kb)', fontsize=7)
